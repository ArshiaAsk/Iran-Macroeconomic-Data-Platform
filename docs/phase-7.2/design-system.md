...
| Native `st.caption` direction (shell-wide) | **Resolved by P2 (Wave H)** — a shell-level rule `[data-testid="stMainBlockContainer"] [data-testid="stCaptionContainer"] { direction: rtl; text-align: start; }` makes every caption in the main block read RTL. It replaced the scoped `coverage_footnote` hook (Task 32), which it fully covers (the Overview footnote is byte-identical) | A native `st.caption` inherits the main block's LTR direction, so a Persian sentence hugged the left edge. The rule is scoped to the main block, so the sidebar's captions are untouched; it is not a component rule, so it lives in the shared-component hook block beside the other shell rules |
