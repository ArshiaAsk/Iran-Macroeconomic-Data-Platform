...
| Bar-list domain order | The rows keep `available_domains`'s order (alphabetical by domain slug), not the mockup's count-descending order (Task 31) | Task 31's build says the rows come from `available_domains` and does not specify a sort; the ordering difference is flagged for the Task 34 owner review rather than decided here |
