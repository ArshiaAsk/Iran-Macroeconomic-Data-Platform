...
class CollectionConfig(BaseSettings):
    ...
    user_agent_rotation: bool = Field(
        default=True, alias="USER_AGENT_ROTATION"
    )
...
class APIConfig(BaseSettings):
    ...
    world_bank_url: str = Field(
        default="https://api.worldbank.org/v2", alias="WORLD_BANK_API_URL"
    )
    imf_url: str = Field(
    ...
    eia_url: str = Field(
        default="https://api.eia.gov/v2", alias="EIA_API_URL"
    )
